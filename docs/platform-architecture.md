...
├── estimate.html                  # Bidding & estimating tool (~23,700 lines HTML/JS)
...
│   └── estimate.css               # All CSS for estimate.html (~8,050 lines)
├── assets/
...
**estimate.html + estimate.css — Bidding & Estimating Tool (~23,700 lines HTML/JS + ~8,050 lines CSS)**
...
- **ENH (Enhancement) Division** ✅ **BUILT** (Mar 2026): Full Enhancement division implementation extending the MNT estimating engine with division-specific catalogs, takeoffs, material/subcontractor line items, plant catalog, and milestone scheduling.
  ...
  **Plant Catalog** — standalone plant database, new sidebar nav item:
  ...
  - Frontend: Full CRUD, search/filter, photo upload, CSV import stub, inline size editing. **Dynamic plant categories**: stored in `bidSettings.plantCategories` (JSON array in Google Sheets), defaults to `['Shrub','Tree','Annual','Perennial','Ornamental Grass','Ground Cover']`. All category dropdowns (filter, form, import review) populated dynamically via `getPlantCategories()` / `refreshPlantCategoryDropdowns()`. "+ Add Category" option at bottom of form and import review dropdowns — prompts for name, saves to Google Sheets immediately.
...
- **Two-Tier Estimate Header** ✅ **BUILT** (Mar 2026): Replaced cluttered single-row header with a state-driven two-tier layout for the builder view:
  ...
  - **Bottom tier (actions)**: State-dependent buttons only — Draft: Save as Template, Save Estimate, Finalize; Revision: Save Draft, View Current PDF, Update Contract; Finalized: Revise Estimate, View Schedule, PDF actions, primary action chain: Generate PDF → Send Contract → View in Contracts (after signed, before tickets — navigates to contract detail for two-phase schedule generation) → Generate Signed PDF
...
- **Property Takeoff System** with Attentive.ai Excel import (SheetJS):
  ...
  - **Add Section Modal**: Manual builder for custom takeoff sections (split, value, calc types). No tabs — direct form. MARVIN chat panel handles AI-driven section creation via `createSection` action cards.
  - **MARVIN Conversational AI Panel**: Full slide-out right panel (inside `.app-container` as flex sibling of `.main-content`). FAB button (bottom-right, z-index 50) opens 400px aside panel (z-index 200). Supports response types: `text` (plain chat), `action.setField` (update estimate fields via `MARVIN_FIELD_MAP`), `action.createSection` (add takeoff sections with rich preview card showing name/type/unit/rows/calc formula), `action.navigate` (switch views), `action.bulkUpdate` (batch-modify existing catalog entries — e.g., set supplier on all plants). Rich context injection via `buildMarvinContext()` sends a comprehensive platform snapshot on every request. `ensureMarvinData()` is called before every context build to pre-load lazy data (plantCatalog, properties) that may not be in memory if the user hasn't visited those views yet. **Platform-wide (always sent):** all estimates/bids (`savedBids[]` — status, bidTotal, contractId, services), all contracts if loaded (`allContracts[]` — dates, payments, crew), all properties with enrichment (bid count, contract count, hasActiveContract, contactName), contacts, reminders, service catalog, plant catalog (all plants with sizes/suppliers for bulk update context), full bid settings (rates, markups, divisions). Knowledge base always included. **Active estimate (when builder view is open OR estimate has data):** property info, measurements, takeoffs, services with line items, contract settings, rate overrides, calculated totals, tier breakdowns. `hasActiveEstimate()` returns true when `currentView === 'builder'` (even for empty new estimates) or when the estimate has a property address, services, or feature measurements. **View-specific:** Schedule tickets (`schedTicketsCache[]`), Contract Detail tickets (`scheduleTickets[]`). System prompt includes Central Florida landscape domain knowledge (production rates, typical values, pricing strategy). System prompt explicitly labels context as "Platform Data" with field-by-field descriptions and instructs MARVIN to never claim it lacks access — answer from JSON data first, note specifically what's missing if anything. Responses are natural conversational text by default; JSON only for actions. `formatMarvinText()` renders light markdown (bold, bullet lists) in assistant bubbles. 4096 max tokens. Conversation persists across view changes. CSS responsive: overlays on viewports < 1200px. Panel state: `marvinChatHistory[]`, `marvinPanelOpen`, `marvinIsThinking`, `marvinTypewriterActive`. Uses `mode:'chat'` parameter to Lambda.
    - **Typewriter Text Effect**: Responses render word-by-word instead of appearing all at once. `typewriterEffect()` splits response on whitespace, renders 2 tokens per 30ms tick (~66 words/sec), re-runs `formatMarvinText()` on growing text so markdown renders correctly throughout. Blinking cursor (`.typing::after` with `marvin-cursor-blink` animation) visible during animation. Action cards appear only after text completes. `skipTypewriter()` cancels animation and renders full text instantly — called automatically when user sends a new message during active animation. State tracked in `marvinTypewriterActive` object (timer, DOM refs, resolve callback).
    - **Morning Briefing**: First MARVIN panel open of the day triggers an automatic briefing. `checkMorningBriefing()` is called from `toggleMarvinPanel()` when the panel opens — checks `localStorage('marvinLastBriefingDate')` vs today, skips if already briefed or if a conversation is in progress. Fires `sendMarvinBriefingMessage()` — a programmatic chat request (no user bubble shown) with a prompt requesting concise 3-5 line briefing: weather for central Florida (via web search), Knowledge Base reminders matching today, and noteworthy open items. Functions: `checkMorningBriefing()`, `sendMarvinBriefingMessage()`. Graceful error fallback message on network failure.
    - **Identify Mode**: Interactive element identification tool. Crosshair icon button (`my_location`) in panel header toggles identify mode via `toggleMarvinIdentifyMode()`. When active: `body.marvin-identify-active` class enables crosshair cursor on all elements (except panel itself), green inset glow on `.main-content`, button gets `.active` class (green tint). Floating highlight div (`.marvin-identify-highlight`, z-index 10001) follows hovered elements using `getBoundingClientRect()` with green border and subtle tint + page dimming via box-shadow. Tooltip (`.marvin-identify-tooltip`) shows `tag#id.class` above hovered element in green monospace text. Document-level capture-phase event listeners prevent underlying click handlers from firing. Click captures element context via `captureElementContext()` — collects tag, id, classes, text (200 char cap), data attributes, onclick handler string, 5-level parent chain (tag/id/classes), `currentView`, and closest meaningful container (`.view`, `.card`, `.modal`, etc.). `sendMarvinIdentifyMessage()` shows user bubble with short summary, sends full context JSON to MARVIN API asking for 3-5 sentence practical explanation, renders response with typewriter animation. Exits after one click; ESC key also exits. Closing panel via `toggleMarvinPanel()` auto-exits identify mode. Skips body/html tags, modal-overlay backdrops, and config-panel-overlay backdrops (lets clicks pass through to modal content inside). State: `marvinIdentifyMode` boolean. Functions: `toggleMarvinIdentifyMode()`, `enterMarvinIdentifyMode()`, `exitMarvinIdentifyMode()`, `marvinIdentifyHover()`, `marvinIdentifyMouseOut()`, `marvinIdentifyClick()`, `marvinIdentifyKeydown()`, `captureElementContext()`, `sendMarvinIdentifyMessage()`. CSS classes: `.marvin-identify-highlight`, `.marvin-identify-tooltip`, `.marvin-panel-btn.active`, `body.marvin-identify-active`.
  - **MARVIN File Upload & Smart Import**: Users can attach CSV, Excel (.xlsx/.xls), and PDF files directly in the MARVIN chat panel via paperclip button or drag-and-drop. Files are parsed client-side (SheetJS for spreadsheets, pdf.js v3.11.174 for PDFs). **Deterministic client-side auto-mapper** (`autoDetectImportMapping()`) handles spreadsheet column mapping entirely in the browser — no LLM involved. The function scores each of the five import targets (Plant Catalog, Contacts, Item Catalog, Service Catalog, Properties) by matching headers against keyword lists and field alias dictionaries. Two-pass matching: exact alias match first, then fuzzy substring match. Each target has a `fields` object mapping target field names to arrays of known aliases (e.g., `unitCost: ['price', 'unit price', 'cost', 'unit cost', 'each', 'ea', 'cost each']`). When the best target scores ≥ 2, the import card renders immediately with pre-filled column mappings — users see the result instantly without waiting for an API call. If auto-detection fails (score < 2 or no mappings found), the file is left attached and the user can send it to MARVIN for help. PDF text (up to 8,000 chars) is still sent to Lambda in `context.attachedFile` for MARVIN analysis since PDF structure is too varied for deterministic parsing. Users can adjust mappings via dropdowns before clicking "Import N rows". Each target has duplicate detection (plant by name, contact by email/name, item by name+division, service by name, property by normalized address). Import progress shown in-button ("Importing 12/47..."). Post-import: summary message with navigation link ("View Plant Catalog →"), 60-second undo timer for created items (not supported for Item Catalog — `deleteItem` requires rowIndex). PDF imports include "Download as CSV to review first" link. Multi-sheet Excel files show a sheet picker dropdown. File size limit: 10MB. Empty/malformed rows (< 30% columns filled) filtered out. **Smart header detection**: `parseMarvinSheet()` auto-detects the actual header row by scanning the first 15 rows and selecting the one with the most filled text columns (numbers excluded from count). This handles common spreadsheet patterns where row 1 is a title (e.g. "Plant Schedule") and actual column headers are on row 3+. Section labels (rows with only 1-2 filled columns) are filtered out by the 30% threshold calculated against the true header width. **Import Review Step** (two-click flow): Clicking "Import N rows" no longer immediately commits — instead it expands the panel, hides mapping dropdowns, and renders a full review table showing ALL rows with checkboxes and editable data. All text cells are inline editable `<input>` fields — users can click any cell to override the value before import. Category column remains a dropdown for `plantCatalog`. **Value flagging**: `mirFlagValue()` detects ambiguous values (size ranges like "30-45 gallon", price ranges, "X or Y" patterns) and highlights them with an orange warning style + tooltip explaining the issue. Flags clear automatically when the user edits the value. A warning banner at top shows total flagged count. User can uncheck rows (e.g., sod, section headers, junk data), then clicks "Confirm Import N rows" to commit only selected rows. For `plantCatalog` target: each row gets a category dropdown (Shrub, Tree, Annual, Perennial, Ornamental Grass, Ground Cover); a yellow warning banner appears if no category column was mapped. Select all / deselect all toggle in controls row. `applyMarvinAction()` handles the two-click flow: first click calls `showImportReviewStep()`, second click calls `applyMarvinImportFiltered()` which passes `_filteredMappedRows` into `applyMarvinImport()`. State: `marvinAttachedFile`, `marvinWorkbook`, `marvinLastImportIds`, `marvinUndoTimer`. Functions: `initMarvinDragDrop()`, `processMarvinFile()`, `processMarvinSpreadsheet()`, `processMarvinPDF()`, `parseMarvinSheet()`, `handleMarvinSheetChange()`, `clearMarvinFile()`, `handleMarvinFileSelect()`, `autoDetectImportMapping()`, `renderMarvinImportPreview()`, `updateMarvinMapping()`, `downloadExtractedCSV()`, `mirFlagValue()`, `renderMarvinImportReviewTable()`, `toggleAllReviewRows()`, `toggleReviewRow()`, `updateReviewRowCategory()`, `handleReviewCategorySelect()`, `updateReviewRowField()`, `updateReviewCount()`, `showImportReviewStep()`, `applyMarvinImportFiltered()`, `applyMarvinImport()`, `importRowByTarget()`, `importPlantRow()`, `importContactRow()`, `importItemCatalogRow()`, `importServiceCatalogRow()`, `importPropertyRow()`, `showMarvinUndoImport()`, `executeMarvinUndo()`. CSS classes: `.marvin-attach-btn`, `.marvin-file-chip`, `.marvin-sheet-picker`, `.marvin-drop-overlay`, `.marvin-import-meta`, `.marvin-mapping-row`, `.marvin-mapping-select`, `.marvin-import-preview-table`, `.marvin-csv-download-btn`, `.marvin-undo-banner`, `.mir-warning`, `.mir-controls`, `.mir-select-all`, `.mir-count`, `.mir-table-wrap`, `.mir-header`, `.mir-row`, `.mir-excluded`, `.mir-cell`, `.mir-cell-cb`, `.mir-cell-cat`, `.mir-cell-warn`, `.mir-cat-select`, `.mir-input`, `.mir-input-warn`.
  - **MARVIN Bulk Update**: `bulkUpdate` action type allows MARVIN to batch-modify existing catalog entries. When the user asks to change a field on multiple items (e.g., "set supplier to County Line on all plants", "change category to Tree for all oaks"), MARVIN returns a `bulkUpdate` action card. Data format: `{ target: "plantCatalog", targetLabel: "Plant Catalog", updates: { supplier: "County Line" }, filter: {}, affectedCount: N }`. Supported targets: `plantCatalog`. Updatable fields: `supplier` (applied to all sizes on each plant), `category`, `notes`. `filter` narrows scope (e.g., `{ category: "Shrub" }`); empty = all items. `affectedCount` is computed from `context.plantCatalog` matching the filter. Action card shows `edit_note` icon with blue theme (`.bulk-update`), detail summary of changes, "Update N items" button. `applyMarvinBulkUpdate()` iterates matching plants, POSTs `savePlantEntry` for each with progress in button text, reloads catalog, shows summary toast + assistant chat message. Functions: `applyMarvinBulkUpdate()`. CSS class: `.marvin-action-icon.bulk-update`.
  - **MARVIN Knowledge Base**: Persistent company instructions stored in `bidSettings.marvinKnowledgeBase` (saved to Google Sheets via `updateBidSettings`). Full-width textarea in Settings view under "MARVIN Knowledge Base" card. Text is injected into every chat request via `buildMarvinContext()` → `context.knowledgeBase` → Lambda system prompt section "Company Knowledge Base (from Settings — always follow these)". MARVIN can self-edit the knowledge base via `updateKnowledgeBase` action type — triggered by "remember...", "from now on...", "always/never..." phrases. Action card shows "Save to Notes" / "Remove" buttons. `applyMarvinKBUpdate()` appends/removes lines, updates Settings textarea, and auto-saves to Google Sheets. Allows teaching MARVIN company-specific preferences (minimum bids, preferred equipment, supplier pricing, route travel times, seasonal standards) conversationally.
  - Lawn: equipment splits (48" Mower, 21" Mower, String Trimmer) with percentage allocation. Difficulty splits (Easy/Med/Hard) are a property-level attribute, not a takeoff section. `buildEquipmentSelect()` renders equipment dropdowns with HTML-escaped `&quot;` for item names containing double quotes (e.g., `21" Mower Walk`). Percentage input is saved regardless of whether rawSF is set (only SF→percentage conversion requires rawSF > 0).
...
- **Typed E-Signature Flow (ESIGN Act / UETA compliant)**: Full contract signing workflow without DocuSign or login:
  ...
  3. Customer opens `sign.html?token=<uuid>` — standalone page (no login required). Backend verifies PDF hash integrity (re-computes SHA-256 from Drive file, compares to stored `pdfHash`). If mismatch detected, sign.html shows tamper error: "This contract document has been modified since it was sent."
  ...
  11. **Stale cache fix**: Both bid-load paths in estimate.html always fetch fresh contract data from backend (not from `allContracts` cache) to ensure `signedName`, `signedAt`, `signedPdfUrl` etc. are current after signing
  - **Legal enforceability (ESIGN Act / UETA compliance)**: The system records seven elements: (1) **Intent to sign** — consent checkbox + "Sign & Accept" button click, (2) **Consent text** — stored verbatim in `consentText` column, (3) **Identity** — signedName, signedIP, signedUserAgent, (4) **Timestamp** — server-side ISO timestamp in `signedAt`, (5) **Document integrity** — SHA-256 hash of contract PDF computed at send time (stored in `pdfHash`, format: `SHA-256:<hex>`), verified when customer opens signing page, (6) **Association** — UUID `signingToken` ties signature to specific contract, (7) **Record retention** — all data in Sheets + signed PDF on Drive. **Company signature audit trail**: `companySigner` (name), `companySignedAt` (ISO timestamp), `companySignedIP` (IP address from estimator's browser)
  - **Signing status lifecycle**: `unsent` → `sent` (email sent) → `viewed` (opened page) → `signed` (customer signed) → `revised` (contract revised after signing, signature voided) → `sent` → ... (re-signing cycle)
...
- **Estimates List Filtering**: The Estimates sidebar view filters out bids whose linked contract has `signingStatus === 'signed'`. Once signed, the estimate disappears from Estimates immediately — no longer requires `ticketsGenerated === 'Yes'`. These bids live in the Contracts view instead. When all bids are contracted, an "All Caught Up" empty state shows: "All estimates have been signed and moved to Contracts." Drafts, sent, finalized-unsigned, and revision bids always appear.
- **Contract Lifecycle UX**: Pre-signing workflow stays entirely in Estimates; post-signing workflow moves to Contracts. The Contracts list (`renderContractsList()`) filters to only show contracts where `signingStatus === 'signed'` OR `status === 'terminated'` — unsigned/unsent/finalized-only contracts do not appear. Empty state reads "No Signed Contracts Yet" with guidance to manage signing from Estimates. The "View Schedule" button in the estimate header (`renderEstimateHeader()`) is gated behind `isSigned && hasTickets`; when `isSigned && !hasTickets`, a "View in Contracts" button navigates to the contract detail instead. Revised contracts (where signing fields are reset) disappear from Contracts until re-signed. **"Needs Schedule" badge**: Contract cards in the list show an orange `.needs-schedule-badge` when the contract is signed but `ticketsGenerated !== 'Yes'` and not terminated — alerts the user that Generate Schedule still needs to be run.
- **Two-Phase Schedule Generation (Preview → Finalize)**: After a contract is signed and appears in the Contracts view, the contract summary shows a "Generate Schedule" button (`generateScheduleFromContract()`). Clicking it fetches the full estimate data from Drive via `getReportData`, runs `generateAllTickets()` locally, assigns temporary `preview-N` ticket IDs, calculates earned value/internal cost proportionally, and loads the tickets into the calendar view in **preview mode** (`schedulePreviewMode = true`). Preview tickets render with dashed borders (`.preview-ticket` CSS class). The user can navigate months, drag-and-drop tickets to adjust dates (drag-drop in preview mode skips the backend POST), and review the schedule. A sticky "Schedule Finalize Bar" appears at the bottom with Cancel and "Finalize Schedule" buttons. Clicking "Finalize Schedule" (`finalizeSchedule()`) confirms, POSTs all tickets to the backend via `saveTickets`, marks the contract `ticketsGenerated: 'Yes'`, exits preview mode, and reloads the contract detail to get real ticket IDs. "Cancel" (`cancelSchedulePreview()`) discards the preview, clears tickets, and returns to the contract summary. State variables: `schedulePreviewMode` (boolean), `schedulePreviewEstimate` (estimate data). Preview mode is also reset when navigating away via `showContractsList()`.
- **Contract Summary Page**: Clicking a contract in the Contracts view opens a summary panel instead of jumping directly to tickets. The summary shows: property address with status badge (Signed/Active/Terminated/Needs Re-signing/Unsent), action buttons (View Estimate, View Tickets, View Contract PDF, View Signed PDF, Generate Schedule, Terminate Contract), a details grid (Contract ID, date range, monthly/annual values, payment terms, crew, contacts, billing address, signing status, ticket counts), and a services list derived from the linked bid's sections. "Generate Schedule" button appears when contract is signed but tickets have not yet been generated (hidden when terminated or tickets already exist). "View Tickets" navigates to the existing service/calendar sub-views. "View Estimate" opens the linked bid in the builder via `openSavedEstimate()`. PDF buttons open URLs in new tabs (hidden when no URL). Back navigation from tickets returns to the summary panel.
- **Contract Termination**: Red "Terminate Contract" button on the contract summary (hidden if already terminated). Opens a dialog with effective date (defaults to today) and optional reason textarea. On confirm: POSTs `terminateContract` to backend, which sets status to `terminated`, writes `terminatedDate`/`terminationReason`, and deletes future scheduled tickets. Completed/skipped tickets are preserved. Terminated contracts show a red "Terminated" badge and no terminate button.
- **Daily Crew Capacity & Calendar Hours Badges**: Calendar view shows total labor man-hours (excluding travel) on each day across **all contracts**, not just the current one. A small badge in the top-right of each calendar cell displays `X.X hrs`. Days exceeding the configurable capacity threshold get a light red background (`.over-capacity` CSS class) and a red hours badge. The threshold is stored as `dailyCrewCapacity` in `bidSettings` (default: 33 man-hours), configurable via "Daily Crew Capacity (Man-Hours)" input in the Settings page Labor Settings card. State: `allTicketsByDate` object keyed by `YYYY-MM-DD` containing arrays of `{totalEstHours, travelHours}` from all contracts. Function: `loadAllTicketsForMonth()` fetches all tickets for the calendar's visible 6-week range (42 days) via `getTickets?startDate=...&endDate=...` (no contractId — returns all contracts). Called on contract open, month navigation, and schedule generation. In preview mode, preview tickets (IDs starting with `preview-`) are added to daily totals since they aren't in `allTicketsByDate` yet. Day detail panel shows a summary bar: `X.X hrs scheduled — capacity: Y hrs (Z over/available)` with green/red coloring. **Hours badge totals use `totalEstHours` only** — travel hours are excluded so the badge matches the sum of individual ticket hours displayed. CSS: `.calendar-day-hours` (green badge), `.calendar-day-hours.over-capacity` (red badge), `.calendar-cell.over-capacity` (red background), `.calendar-cell.other-month.over-capacity` (muted red).
...
- **Contacts (Lite CRM)**: Lightweight contact management built into estimate.html as a placeholder until HubSpot integration. Contacts have lifecycle stages (Lead → Prospect → Customer), are linked to estimates via `contactId`, and auto-promote to "Customer" when an estimate is finalized. Searchable by name, email, phone, address. Contact picker in the estimate builder auto-fills property address. Contact profile shows linked estimates and linked contracts (with View PDF links when available). Standalone `.contacts-grid` layout (8px card gap, individual card borders/radius). `setTimeout`-based scroll-to-top on all view switches and data loads (fires after browser layout completes). Stored in a "Contacts" Google Sheet (fields: contactId, firstName, lastName, email, phone, company, billingAddress, propertyAddress, stage, source, notes, createdAt, updatedAt, displayName). **displayName** is an optional override for how a contact's name appears everywhere — used for co-owners on residential accounts (e.g. "John & Jane Smith"). `getContactDisplayName()` checks displayName first, then falls back to firstName + lastName, then company/email. All finalize paths and PDF metadata use `getContactDisplayName()`. The `contactId` foreign key pattern survives the HubSpot migration — the field becomes `hubspot_contact_id` but the linking pattern is the same.
- **Property Address — Single Source of Truth**: The property picker dropdown (`#estimate-property-picker`) is the sole source of the estimate's property address. The manual `#prop-address` text input has been removed. All address writes go through `currentEstimate.propertyAddress` set by the property picker's `onEstimatePropertySelect()`. Attentive XLSX uploads no longer write the address — they import measurements only. A mismatch warning toast fires when the Attentive report's parsed address doesn't match the selected property (substring comparison, case-insensitive), alerting the user to verify they uploaded the correct file.
- **Address Autocomplete** *(planned)*: Google Places Autocomplete or AWS Location Service on address input fields (`contact-billing-address`, `contact-property-address`). Dropdown suggestions as user types, auto-fills full formatted address. Biased toward Florida service area. **Options**: Google Places API ($2.83/1K requests, $200/mo free credit, best US residential data) or AWS Location Service ($4/1K requests, 10K/mo free, keeps everything in existing AWS account). Requires API key with HTTP referrer restrictions.
- **Properties View**: Central hub for every service address in the system. A property is the core operational entity — a single customer can have multiple properties, and a single property can have maintenance contracts, enhancement projects, and construction jobs running simultaneously. Each property card shows address, lot size, linked contact(s), and status indicators for active contracts. Clicking a property opens its profile with the following sections:
  - **Measurements (Attentive Takeoffs)**: All property measurements sourced from Attentive.ai takeoff reports or entered manually. Displays lawn SF, mulch bed SF, hedge SF/LF, hard/soft edge LF, tree count, irrigation zones, driveway/pavement SF, and all perimeters in an excel-style grid (mower-table layout with blue values). Difficulty splits are NOT on the property page — they are set per-estimate in the estimate builder. `measurement_source` badge indicates origin (`attentive`, `manual`, `polygon`, `gps_trace`, `drone`). Link to the original Attentive report file when available. Drag-and-drop Attentive .xlsx upload zone on property modal auto-parses measurements and populates the grid. These measurements drive every estimate — changing a measurement here propagates to future bids. In the current prototype, measurements live in the estimate's Property Setup tables (mower splits, edge splits, etc.); the Properties view centralizes them so they're entered once and reused across all estimates for that address.
  ...
  **Data model (IMPLEMENTED)**: Properties are first-class entities stored in the Properties sheet with PROP-0001 format IDs. Each property has full address fields, crew assignment, measurements (lot size, lawn, edges, mulch beds, hedge, driveway, sidewalk, trees, irrigation zones), measurement source tracking (attentive/manual/polygon/gps_trace/drone), Attentive report URL, gate code, and notes. Difficulty splits are stored on the estimate, not the property. Contacts link to properties via many-to-many PropertyContacts junction sheet with role designations (Owner/Co-Owner/Property Manager/Billing Contact/Tenant/Other) and optional isPrimary flag for Co-Owner role. Sub-contractors tracked per property in SubContractors sheet. Properties loaded from backend via `getEstimatingProperties` with fallback to aggregation from contacts/bids/contracts when backend sheet is empty. Property picker in estimate builder auto-fills address, measurements, and difficulty splits into new estimates.

  **State**: `properties[]`, `currentProperty`, `propertySearchQuery`, `propertyFilter`, `propertyContacts[]` (junction links), `propertySubContractors[]`, `selectedLinkerContactId`, `pendingSubContractFile`. Functions: `normalizeAddress()`, `aggregateProperties()`, `loadProperties()`, `loadPropertiesFromBackend()`, `enrichPropertiesWithBidsContracts()`, `filterProperties()`, `renderPropertiesList()`, `openPropertyProfile()`, `showPropertiesList()`, `openPropertyModal()`, `closePropertyModal()`, `savePropertyFromModal()`, `deleteCurrentProperty()`, `getPropertyDataFromModal()`, `togglePropertySection()`, `openContactLinker()`, `closeContactLinker()`, `renderContactLinkerResults()`, `selectLinkerContact()`, `saveLinkFromLinker()`, `toggleLinkerPrimary()`, `unlinkContact()`, `renderPropertyLinkedContacts()`, `renderPropertySubContractors()`, `openSubContractorModal()`, `closeSubContractorModal()`, `saveSubContractorFromModal()`, `deleteSubContractor()`, `handleSubContractFileSelect()`, `clearSubContractFile()`, `renderPropertyProjects()`, `createEstimateFromProperty()`, `populateEstimatePropertyPicker()`, `onEstimatePropertySelect()`, `getLinkedPropertiesForContact()`, `resolveContactByRole(propertyId, preferredRole, fallbackRole)` (role-aware contact lookup — searches PropertyContacts junction for preferred role, with Co-Owner+isPrimary fallback when resolving Owner, then fallbackRole, then first linked contact), `getContactDisplayName(contact)` (returns displayName if set, else firstName+lastName, else company/email). CSS: `.property-measurements-grid`, `.property-source-badge` (with source-specific color classes), `.property-modal-section-header`, `.sub-contractor-card`, `.contact-linker-item`, `.property-linked-contact`, `.property-picker-wrapper`, `.division-badge`, `.btn-danger-text`, `.pm-grid-input` (blue editable inputs matching mower-table style).
...
- **Schedule View (Route Management)**: Three display modes — day view with property stop cards and drag-drop reordering (`schedDrop()` + `saveRouteOrder()`), week calendar grid with drag-to-reschedule between dates, month calendar with ticket dots. Crew filter dropdown. Stop detail panel with earned value, margin, services. **Ticket status styling**: All three views apply status-based CSS classes (`status-scheduled`, `status-completed`, `status-skipped`, `status-rescheduled`). Completed tickets render with reduced opacity and line-through text in all views. Skipped tickets also get opacity + line-through. **Needs-reschedule styling**: Tickets with `needsReschedule = true` get orange shading (`.needs-reschedule` class) — background `#FEF7E0`, text `#E8710A`, orange border — applied to `.calendar-ticket`, `.sched-week-ticket`, and `.sched-month-ticket`. When the ticket is dragged to a new date (clearing `needsReschedule`), it returns to normal green styling. Functions: `loadScheduleView()`, `renderSchedDay()`, `renderSchedWeek()`, `renderSchedMonth()`, `showSchedTicketDetail()`, `rescheduleFromDetail()`, `skipFromDetail()`.
...
- **Properties View**: First-class property entities loaded from backend Properties sheet via `getEstimatingProperties`. Each property has full measurements, difficulty splits, crew assignment, and sub-contractor tracking. Falls back to `aggregateProperties()` when backend sheet is empty. Nav item between Contacts and Invoices with house icon. "Add Property" button in toolbar. List view with search + filter (All / Active Contract / No Contract). Each property card shows address, primary contact name (from junction links or address-match fallback), contract status badge, and monthly value. Click to open property profile showing: **Measurements** (from property record with source badge — Attentive/Manual/Polygon/GPS/Drone — and Attentive report link), **Linked Contacts** (from PropertyContacts junction with role badges, unlink buttons; legacy address-match contacts also shown), **Sub-Contractors** (cards with company, service type, edit/delete), **Estimates** (all bids for this property), **Contracts** (recurring maintenance only — filtered to `jobType !== 'work_ticket' && (!division || division === 'MNT')`), **Projects** (one-off work: work tickets, ENH, CON, IRR contracts — clickable cards with project name, division badge, status, dates, value; click opens `openContractDetail()` full project page with tickets/calendar). New Estimate, Edit Property, and Delete buttons in profile header. **New Estimate from Property**: "New Estimate" button stashes `_pendingPropertyId`, enters standard job type → template picker flow; after builder opens, `onEstimatePropertySelect()` auto-fills address, measurements, takeoffs, linked contact, and services. **Property Modal**: Full add/edit form with address (auto-parsed into street/city/state/zip from Attentive, zip auto-geocoded via Nominatim), details (type/PIN/gate code), crew assignment (dropdown populated from `getCrews`), drag-and-drop Attentive .xlsx upload zone (parses measurements and auto-populates grid), collapsible measurements section with excel-style grid (mower-table layout, blue `.pm-grid-input` fields), notes. Auto-generated 4-digit PIN with Hamming distance ≥ 2 from all existing PINs (single-digit typo can never match another property). Difficulty splits removed from property — set per-estimate instead. **Contact Linker**: Overlay to search and link contacts with role selection (Owner/Co-Owner/Property Manager/Billing Contact/Tenant/Other). When Co-Owner role is selected, a "Primary contact (billing & contract signing)" checkbox appears (checked by default). Primary flag is stored as isPrimary='Yes' in PropertyContacts junction. Linked contacts display shows "Co-Owner · Primary" badge for primary co-owners. **Sub-Contractor Modal**: Add/edit company, service type, phone, email, contract notes. **Estimate Builder Integration**: Property picker dropdown auto-fills address and measurements into new estimates, and auto-selects the Owner-role contact (via `resolveContactByRole()`). Property cards also prefer Owner role for the primary contact display. **Role-Aware Contact Resolution**: `resolveContactByRole(propertyId, preferredRole, fallbackRole)` searches PropertyContacts junction for the preferred role, falls back to fallbackRole, then first linked contact. When resolving 'Owner', the chain is: Owner → Co-Owner with isPrimary='Yes' → any Co-Owner → fallbackRole → first linked contact. Same Co-Owner fallback applies when fallbackRole is 'Owner'. Used at finalize time to resolve billing contact separately from the signing contact. Property card display and estimate auto-select also use this Owner → primary Co-Owner → Co-Owner fallback chain. **Billing Contact on Contracts**: At finalize (all three paths: new contract, revision, work ticket), the system resolves a billing contact via `resolveContactByRole(propertyId, 'Billing Contact', 'Owner')` and writes `billingContactName` + `billingContactEmail` to the contract row. `generateInvoiceBatch()` prefers billing contact for invoice delivery while `sendContractForSigning` continues using the primary Owner contact.
...
- **Contracts Sheet (services column)**: `Services JSON` — JSON array of service objects (`{name, category, billingTier, annualTotal}`) stored at contract creation from estimate.html. Used by `generateInvoiceBatch()` to build division-aware invoice line items grouped by category (Maintenance, Enhancement, Irrigation, Construction) instead of a single "Monthly Maintenance" line.
...
| `getProperties` | Crew | Returns all properties with address, crew, phone, pin (backward-compat subset from expanded Properties sheet) |
...
| `getPropertyContacts` | Estimating | Returns all links from PropertyContacts junction sheet (linkId, propertyId, contactId, role, isPrimary) |
...
| `saveContact` | Estimating | Creates a new contact in the Contacts sheet with auto-generated C-{timestamp} ID. Fields include displayName (optional — for co-owners, e.g. "John & Jane Smith"). Auto-creates displayName column on older sheets |
| `updateContact` | Estimating | Updates an existing contact by contactId. Writes displayName column by header index (safe for old sheets without the column) |
| `deleteContact` | Estimating | Deletes a contact by contactId |
| `saveProperty` | Estimating | Creates new property in Properties sheet, returns PROP-0001 format ID |
| `updateProperty` | Estimating | Updates property by propertyId, preserves createdAt |
| `deleteProperty` | Estimating | Deletes property + its junction links + its sub-contractors |
| `linkContactToProperty` | Estimating | Adds row to PropertyContacts junction (propertyId, contactId, role, isPrimary) |
| `unlinkContactFromProperty` | Estimating | Removes row from PropertyContacts by linkId or propertyId+contactId |
...
| `createContract` | Estimating | Creates contract row with fields: bidId, propertyAddress, assignedCrew, preferredDay, startDate, endDate, contractMonths, monthlyPayment, paymentTerms, contractValue, contactName, contactEmail, billingAddress, billingContactName, billingContactEmail, pdfUrl, pdfFileId. Auto-creates new columns (including Billing Contact Name/Email) on existing sheets |
| `updateContract` | Estimating | Updates existing contract row by contractId — all fields including paymentTerms, contractValue, contactName, contactEmail, billingAddress, billingContactName, billingContactEmail, pdfUrl, pdfFileId. Supports `revisionReset: true` flag to clear signing fields (`signingStatus` → `'revised'`, clears `signedName`, `signedAt`, `signedPdfUrl`, `signedPdfFileId`, `pdfHash`) when a signed contract is revised |
| `terminateContract` | Estimating | Sets contract status to `terminated`, writes `terminatedDate` and `terminationReason` columns (auto-created if missing), then calls `deleteFutureTickets()` to remove future scheduled tickets. Completed/skipped tickets are preserved. Returns `{ success, deletedTickets }` |
...
| `generateInvoiceBatch` | Invoicing | Scans active contracts, creates draft invoices for current billing period (dedup by contractId + period). **Billing contact routing**: prefers `Billing Contact Name`/`Billing Contact Email` from contract, falls back to primary `contactName`/`contactEmail` — allows invoices to go to a billing dept while contract signing goes to the Owner. **Division-aware line items**: parses `Services JSON` column, groups fixed-tier services by category (Maintenance/Enhancement/Irrigation/Construction), creates per-division line items with month name (e.g., "February Maintenance — $500.00"). Falls back to single "Monthly Maintenance" line if no Services JSON. Auto-charges auto-pay contracts via Stripe PaymentIntents, flags open tickets. Returns `{ invoices, openTickets, autoPayResults }` |
...
### What Won't Scale
- Google Sheets as a database (no relationships, slow at volume, concurrent write issues). **Mitigation**: `getInitData` bulk endpoint returns all estimating data in a single request (9 datasets), cutting round-trips from 9+ to 1. Frontend `loadAllData()` processes the combined response with fallback to individual requests on failure.
...
- **MARVIN AI endpoint**: Two Lambda functions serve MARVIN — separate packaging, separate runtimes:
  ...
  - **Personality**: JARVIS-inspired persona with structured "Personality & Voice" section at top of system prompt (core traits, voice rules, good/bad examples) AND a closing "Voice Reminder" at the very end to prevent persona drift. Personality guide covers: calm competence with dry wit, anticipatory behavior, respectful honesty, economy of language, professional warmth. Both `lambda_function.py` and `marvin-stream/handler.py` share identical personality directives.
  ...
  - **Frontend streaming**: `streamMarvinResponse()` in estimate.html — uses `fetch()` + `ReadableStream` with `parseSSEBuffer()` to handle split SSE chunks. Stop button via `AbortController` — `updateMarvinSendButton()` toggles send/stop icon. All three send functions (`sendMarvinMessage`, `sendMarvinBriefingMessage`, `sendMarvinIdentifyMessage`) use streaming when `MARVIN_STREAM_URL` is set, with sync fallback. **Graceful error recovery**: All three catch blocks recover partial streamed text — if MARVIN already streamed a response before an error occurred, the streamed text is displayed instead of a generic error. `streamMarvinResponse()` attaches `marvinPartialText` to thrown errors. When no partial text is available, the actual error message is shown (e.g., "Something went wrong: Stream request failed: 500") rather than the old generic "Sorry, I had trouble processing that." SSE parser logs malformed JSON events to console instead of silently swallowing them.
  - **Custom data tools** (7 tools, defined in `_MARVIN_CUSTOM_TOOLS`): Allow Claude to fetch live data from Google Sheets backend when the browser context doesn't contain the needed data. Each tool maps to a backend GET endpoint:
    ...
    |------|---------------|------------|----------|
    | `get_schedule` | `getTickets` | `start_date`, `end_date`, `crew`, `contract_id` (all optional, defaults to today + 7 days) | Schedule queries when user hasn't visited Schedule view |
    | `get_contracts` | `getContracts` | none | Contract queries when Contracts view not visited |
    | `get_invoices` | `getInvoices` | `status`, `contract_id` (optional) | Invoice and billing queries |
    | `get_production_data` | `getProductionAnalysis` | `start_date`, `end_date`, `crew` (optional) | Crew efficiency and production rate analysis |
    | `get_properties` | `getEstimatingProperties` | none | Property details and measurements |
    | `get_contacts` | `getContacts` | none | CRM contact lookups |
...
- **Template structure**:
  ...
  cloud-function/templates/
    base.html                   # Shared @page rules (default, cover, content), CSS vars, running header element, orphans/widows, footer
    site_report.html            # Photo grid report with per-category bookmarks + running header
    before_after.html           # Comparison report with per-category bookmarks + running header
    contract_residential.html   # 3-page residential contract with logo, bookmarks, running header, formatted dates, green accents
    contract_commercial.html    # 5-6 page commercial contract with 6 section bookmarks, running header, formatted dates, green header bars
    invoice.html                # Invoice PDF template with formatted dates, green rule, green details box accent
    styles/
      common.css                # Shared print CSS (header, info box, green category bars, running header element)
      site_report.css           # 2-column CSS grid, softer photo borders (light-gray, rounded), green note accent
      before_after.css          # BEFORE/AFTER paired layout, softer borders, brand green AFTER label
      contract.css              # Green table headers/info boxes, service description blocks with green left-border, clause orphan control, @font-face Dancing Script
      invoice.css               # Invoice table, green accents, green left-border details box, tabular-nums dates
  ```
...
- [x] Upload Attentive.ai Excel export for a property → auto-parse all measurements (drag-and-drop upload zone on property modal)
- [x] Extract: lot size, lawn SF, mulch bed SF, hedge SF, hard/soft edge LF, tree count, driveway/pavement SF, sidewalk SF
- [x] Store parsed measurements on the property record with `measurement_source = 'attentive'`
- [ ] Store the original Attentive file in Cloud Storage, link on property record
- [x] Manual override: edit any measurement after import (excel-style grid with blue editable inputs)
- [x] Manual entry: for properties without an Attentive report, enter measurements by hand (`measurement_source = 'manual'`)
- Note: Difficulty splits are set per-estimate in the estimate builder, not on the property
...
| PropertyContacts | Junction table for many-to-many property↔contact links: linkId (LINK-timestamp), propertyId, contactId, role (Owner/Co-Owner/Property Manager/Billing Contact/Tenant/Other), createdAt, isPrimary ('Yes'/'' — used with Co-Owner role to designate billing & signing contact) |
...
3. **Finish the prototype pipeline**:
   ...
   - ✅ **Contract finalization UI in estimate.html** — "Finalize Estimate" flow creates contract only (tickets deferred). Flow: Finalize → Generate PDF → Send Contract → Customer signs on sign.html → Navigate to Contracts → "Generate Schedule" (preview) → Review/drag tickets → "Finalize Schedule" (save). Two-phase schedule generation: `generateScheduleFromContract()` loads estimate data, generates tickets locally with preview IDs, opens calendar view in preview mode with dashed-border tickets and finalize bar; `finalizeSchedule()` saves tickets to backend and marks `ticketsGenerated: 'Yes'`. Legacy `generateScheduleFromSigned()` kept for backward compatibility. Revision path still regenerates tickets immediately.
