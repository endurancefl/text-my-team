...
│   ├── lambda_function.py         # AWS Lambda handler (~990 lines)
...
**estimate.html + estimate.css — Bidding & Estimating Tool (~17,835 lines HTML/JS + ~7,180 lines CSS)**
...
- **MARVIN AI endpoint** (`/marvin`): Dual-mode AI endpoint for the estimating tool. Uses Anthropic Claude API (`claude-sonnet-4-20250514`). API key stored in AWS Secrets Manager (`marvin-api-key`) and resolved at deploy time via `{{resolve:secretsmanager:...}}` in SAM template. `anthropic>=0.51.0` in `requirements.txt`. Function: `_handle_marvin()` in `lambda_function.py`.
  ...
  - **Chat mode** (`mode:'chat'`): Conversational assistant with **web search** (Anthropic `web_search_20250305` server-side tool) and **7 custom data tools** for live backend fetching. Accepts `{ "prompt": "...", "history": [], "mode": "chat", "context": {...} }`. Context includes current view, property info, services, contract settings, available views. Returns structured JSON: `{ "type": "text|action", "message": "...", "action": { "type": "setField|createSection|navigate", "data": {...} } }`. System prompt: general-purpose assistant with field IDs, view IDs, section types. 4096 max tokens. Web search config: `max_uses: 5`, `user_location: Orlando, FL`. Claude decides when to search or use tools — no routing logic. Unified continuation loop handles `pause_turn` (web search) and `tool_use` (custom tools) stop reasons with up to 5 total iterations and max 2 tool_use rounds. Tools executed in parallel via `ThreadPoolExecutor`. Multi-block response content parsed by extracting all `text` blocks. Helper functions: `_build_chat_system_prompt(context)`, `_build_section_system_prompt()`, `_parse_chat_response(text)`, `_parse_section_response(text)`, `_fetch_backend(params)`, `_execute_tool(tool_name, tool_input)`, `_execute_tools_parallel(tool_uses)`.
