...
- **Estimates List Filtering**: The Estimates sidebar view filters out bids that are fully contracted (signed + tickets generated). A bid is hidden when its linked contract has `signingStatus === 'signed'` AND `ticketsGenerated === 'Yes'`. These bids live in the Contracts view instead. When all bids are contracted, an "All Caught Up" empty state shows: "All estimates have been signed and moved to Contracts." Drafts, sent, finalized-unsigned, and revision bids always appear.
- **Contract Lifecycle UX**: Pre-signing workflow stays entirely in Estimates; post-signing workflow moves to Contracts. The Contracts list (`renderContractsList()`) filters to only show contracts where `signingStatus === 'signed'` OR `status === 'terminated'` — unsigned/unsent/finalized-only contracts do not appear. Empty state reads "No Signed Contracts Yet" with guidance to manage signing from Estimates. The "View Schedule" button in the estimate header (`renderEstimateHeader()`) is gated behind `isSigned && hasTickets` — it only appears once the customer has signed and tickets have been generated. Revised contracts (where signing fields are reset) disappear from Contracts until re-signed.
- **Contract Summary Page**: Clicking a contract in the Contracts view opens a summary panel instead of jumping directly to tickets. The summary shows: property address with status badge (Signed/Active/Terminated/Needs Re-signing/Unsent), action buttons (View Estimate, View Tickets, View Contract PDF, View Signed PDF, Terminate Contract), a details grid (Contract ID, date range, monthly/annual values, payment terms, crew, contacts, billing address, signing status, ticket counts), and a services list derived from the linked bid's sections. "View Tickets" navigates to the existing service/calendar sub-views. "View Estimate" opens the linked bid in the builder via `openSavedEstimate()`. PDF buttons open URLs in new tabs (hidden when no URL). Back navigation from tickets returns to the summary panel.
...
| `recordSignature` | Signing | Validates token, prevents double-signing, records `signedName`, `signedAt` (server-side), `signedIP`, `signedUserAgent`, `consentText`. Computes PDF SHA-256 hash from Drive file, stores in `pdfHash`. Sets `signingStatus = 'signed'`. Then calls `generateAndEmailSignedPdf()` (non-fatal) to auto-generate signed PDF + email. Returns `{ success, signedPdfGenerated, emailSent }` |
...
- **Current (built)**: Typed e-signature via `sign.html` — customer types name, previewed in Dancing Script cursive. No cost, no external dependency. ESIGN Act / UETA compliant: records intent to sign, consent text, identity (name/IP/user agent), server-side timestamp, document integrity (SHA-256 hash), association (UUID token), and retains all records in Sheets + Drive. Signed PDF regenerated with cursive signature on signature page via WeasyPrint (ReportLab fallback available).
