...
- **Property Takeoff System** with Attentive.ai Excel import (SheetJS):
  ...
  - **MARVIN Knowledge Base**: Persistent company instructions stored in `bidSettings.marvinKnowledgeBase` (saved to Google Sheets via `updateBidSettings`). Full-width textarea in Settings view under "MARVIN Knowledge Base" card. Text is injected into every chat request via `buildMarvinContext()` → `context.knowledgeBase` → Lambda system prompt section "Company Knowledge Base (from Settings — always follow these)". Allows teaching MARVIN company-specific preferences (minimum bids, preferred equipment, supplier pricing, route travel times, seasonal standards) without re-explaining each conversation.
