...
- **Address Autocomplete** *(planned)*: Google Places Autocomplete or AWS Location Service on all address input fields (`prop-address`, `contact-billing-address`, `contact-property-address`). Dropdown suggestions as user types, auto-fills full formatted address. Biased toward Florida service area. **Options**: Google Places API ($2.83/1K requests, $200/mo free credit, best US residential data) or AWS Location Service ($4/1K requests, 10K/mo free, keeps everything in existing AWS account). Requires API key with HTTP referrer restrictions. All 3 address fields are single-line combined inputs — no separate street/city/state/zip parsing needed.
