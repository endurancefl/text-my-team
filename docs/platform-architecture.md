...
| `saveContact` | Estimating | Creates a new contact in the Contacts sheet with auto-generated C-{timestamp} ID. Fields include displayName (optional — for husband/wife pairs, e.g. "John & Jane Smith"). Auto-creates displayName column on older sheets |
...
| `linkContactToProperty` | Estimating | Adds row to PropertyContacts junction (propertyId, contactId, role) |
