...
- **Property Takeoff System** with Attentive.ai Excel import (SheetJS):
  ...
  - **MARVIN Conversational AI Panel**: Full slide-out right panel (inside `.app-container` as flex sibling of `.main-content`). FAB button (bottom-right, z-index 50) opens 400px aside panel (z-index 200). Supports three response types: `text` (plain chat), `action.setField` (update estimate fields via `MARVIN_FIELD_MAP`), `action.createSection` (add takeoff sections), `action.navigate` (switch views). Rich context injection via `buildMarvinContext()` sends current view, property info, features/measurements, takeoff data, full service details with line items, calculated totals (bid total, margin, labor hours, monthly price), tier breakdowns, rate overrides, bid setting defaults, contract settings, schedule data from both Schedule view (`schedTicketsCache[]`, up to 80 tickets with dates, property, services, status, crew, est hours) and Contract Detail view (`scheduleTickets[]`, up to 60 tickets), plus contracts summary (`allContracts[]`, up to 40) and properties summary (`properties[]`, up to 40). System prompt includes Central Florida landscape domain knowledge (production rates, typical values, pricing strategy). Responses are natural conversational text by default; JSON only for actions. `formatMarvinText()` renders light markdown (bold, bullet lists) in assistant bubbles. 4096 max tokens. Conversation persists across view changes. CSS responsive: overlays on viewports < 1200px. Panel state: `marvinChatHistory[]`, `marvinPanelOpen`, `marvinIsThinking`, `marvinTypewriterActive`. Uses `mode:'chat'` parameter to Lambda (vs `mode:'section'` for original modal).
